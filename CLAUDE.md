...
- **个人注记**：本地跑 voiceinterview 模块时 WebSocket 连接经常因超时断开，建议把 `spring.websocket.session-timeout` 设为 `300s`（默认 `60s`），本地测试够用；另外 voiceinterview 的多轮评估 prompt 比较长，ollama 跑 llama3 容易 OOM，换 `qwen2.5:7b` 稳定一些
