...
- **个人注记**：本地跑 voiceinterview 模块时 WebSocket 连接经常因超时断开，建议把 `spring.websocket.session-timeout` 设为 `300s`（默认 `60s`），本地测试够用了
- **个人注记**：knowledgebase 模块向量化文档时，本地 pgvector 维度默认是 1536（OpenAI），换 ollama 的 `nomic-embed-text` 模型时维度是 768，记得同步改 `VectorStore` 的 dimensions 配置，否则插入会报错
